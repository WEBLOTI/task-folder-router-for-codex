...
def install(target, routes):
    ...
    write_json(codex_dir / "task-folder-router.json", {"routes": routes, "require_route_prefix": True})
    ...
    snippet = (
        "\n## Task Folder Router\n\n"
        "- Start each new Codex task with one configured label, such as `project: my-app`.\n"
        "- Work inside the folder created or reused by the router.\n"
...
def main():
    ...
    try:
        ...
        target_root = install(args.target, routes)
